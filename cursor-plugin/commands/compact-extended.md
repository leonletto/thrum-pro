---
description:
  Stage a comprehensive 16-section snapshot, verify it landed, then send /summarize
...
restart file, verify it landed, then send `/summarize` to your own tmux pane. Same
...
```bash
# compact SENDS /summarize to your own pane, so a non-tmux agent has no pane to
...
if [ -z "$SESSION_RAW" ]; then
  ...
  echo "For a non-tmux agent, run /summarize yourself and Read your snapshot by hand afterward."
  exit 1
fi
```
...
`/summarize` fires. This matters more for an extended snapshot: you are staging
...
### 4. Verify your snapshot, then fire /summarize (ONE block)
...
command in place of `/summarize` — see
...
Firing `/summarize` before the Step 3 Write lands compacts a pre-write context and
...
if [ "$SNAPSHOT_OK" = 1 ] && [ "$SESSION_OK" = 1 ]; then
  ...
  thrum tmux send "$SESSION" "/summarize"
else
  echo "HOLDING: not firing /summarize. Report the VERIFY FAILED line(s) above to"
  echo "your coordinator (or the operator if you are top-level) and stop."
  exit 1
fi
```

If EITHER check fails, the block HOLDS and does not fire `/summarize` — holding is
...
rationale). After a successful send, the turn ends; emit no further tool calls or
prose this turn. The send is queued, not synchronous — it fires once your pane
goes idle, which only happens if you stop acting now.

## How resume works

After `/summarize` the SAME process and session survive — the daemon binding is
