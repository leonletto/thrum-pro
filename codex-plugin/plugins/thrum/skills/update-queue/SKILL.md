...
Edit only your OWN queue. Handing work to another agent is a message, never a
write to their queue (see the `using-the-queue` skill).
...
reconcile against and report the delta from. Use `--json` for the tally, not the
text table: the JSON machine surface (`{"bundles": [...]}`) is a stable
