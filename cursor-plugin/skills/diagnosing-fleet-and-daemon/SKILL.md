...
description:
  ...
  daemon, is the daemon alive, where is the daemon log, which log file, searching
  logs, hooklog query, command log, jq returned nothing, empty log search, agent
  looks dead, agent stale, last_seen stale, is this agent alive, schema mismatch,
  cannot downgrade, migration on restart, SIGBUS, WAL poison, 522, database
  locked. Also use BEFORE restarting any daemon — the pre-restart migration check
  lives here.
---
