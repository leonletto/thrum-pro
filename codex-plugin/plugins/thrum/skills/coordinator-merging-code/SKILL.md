...
**Ban `rm -r` as well as `rm -rf`.** **Pasting a fence is not enforcing it.**
...
4. **Delete the merged branch at merge time**, after the push is verified.
   `feature/*` and `fix/*` are one-shot — delete freely. **`agent/*`,
   `salvage/*`, `website-dev` and `release/*` are ongoing or archival — do NOT
   delete.** Deferring this is how 562 stale merged branches accumulated.
