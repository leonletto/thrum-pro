...
if [ "$SNAPSHOT_OK" = 1 ] && [ "$SESSION_OK" = 1 ]; then
  ...
  thrum tmux send "$AGENT" "/summarize"
  SEND_RC=$?
  if [ "$SEND_RC" -ne 0 ]; then
    echo "ERROR: thrum tmux send exited ${SEND_RC} — the daemon-routed self-send failed; it will NOT queue or deliver /summarize."
    ...
    tmux -S "$RAW_TMUX_SOCK" send-keys -t "${SESSION}:0.0" "/summarize" C-m
    FALLBACK_RC=$?
