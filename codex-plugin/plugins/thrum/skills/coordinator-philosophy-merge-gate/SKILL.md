...
8. Use `rm -r`, NEVER `rm -rf`, and use `git worktree remove --force` to drop a
   ...
   and stalls gate sub-agents mid-run waiting on a keystroke. `rm -r` runs free.
   Do NOT widen the ask rule to work around this; that entry is the only
   deletion protection on the box.
...
12. **TEAR DOWN YOUR THROWAWAY WORKTREE WHEN THE GATE ENDS — after two checks,
    ...
    `git worktree remove --force <wt>` (see rule 8 on `rm -r`). Rule 11's hazard
    notice on its own — teardown can destroy uncommitted state — reads as a
    reason NOT to tear down. A warning without a procedure does not produce
    caution; it produces paralysis plus litter.
