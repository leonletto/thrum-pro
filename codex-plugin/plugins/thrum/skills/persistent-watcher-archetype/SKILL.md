...
The script's job is mechanical only: loop, capture each roster member's pane to
a file, and emit one matchable line per cycle. It does not judge or act —
`thrum monitor` matches that line and delivers it to you as a thrum message,
which is your wake signal. Judgment (duties 1-4 above) happens in your own turn
when you read that message, not in the script.
