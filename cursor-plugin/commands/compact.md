---
description:
  Stage a standard 11-section snapshot, verify it landed, then send /summarize to
...
restart file, verify it landed, then send `/summarize` to your own tmux pane.
...
readable after `/summarize` fires. There is no durable-backup step (the worktree is
