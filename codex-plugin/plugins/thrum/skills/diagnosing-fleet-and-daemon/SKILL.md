...
directly. **A missing-file error is not a finding — it is a wrong path until you
have proven the file's absence.** If you are on an old checkout or an unmigrated
install, check both locations before concluding the file doesn't exist.
...
> _Dev-fleet note (ignore if you are running a release build): the fix is a
> boot-seam pool recycle._
...
thinking, or sitting at a permission prompt looks identically "stale" to a dead
one.
